...
#include "ns3/ofswitch-modsule.h"
#include "ns3/openflow-1.3-module.h"
...
int main (int argc, char *argv[])
{
  ...
  Config::SetDefault ("ns3::SDNHelper::UseIdealRrc", BooleanValue (false));
  ...
  Ptr<SDNHelper> SDNHelper = CreateObject<SDNHelper> ();
  Ptr<PointToPointEpcHelper> epcHelper = CreateObject<PointToPointEpcHelper> ();
  SDNHelper->SetEpcHelper (epcHelper);
  SDNHelper->SetSchedulerType ("ns3::RrFfMacScheduler");
  SDNHelper->SetHandoverAlgorithmType ("ns3::NoOpHandoverAlgorithm"); // disable automatic handover
  ...
  // Routing of the Internet Host (towards the SDN network)
  ...
  NetDeviceContainer enbSDNDevs = SDNHelper->InstallEnbDevice (enbNodes);
  NetDeviceContainer muSDNDevs = SDNHelper->InstallMuDevice (muNodes);

  // Install the IP stack on the MUs
  internet.Install (muNodes);
  Ipv4InterfaceContainer MuIpIfaces;
  ueIpIfaces = epcHelper->AssignMuIpv4Address (NetDeviceContainer (muSDNDevs));


  // Attach all MUs to the first eNodeB
  for (uint16_t i = 0; i < numberOfMus; i++)
    {
      SDNHelper->Attach (muSDNDevs.Get (i), enbSDNDevs.Get (0));
    }
      ...
      for (uint32_t b = 0; b < numBearersPerMu; ++b)
        {
          ...
          EpcTft::PacketFiSDNr dlpf;
          dlpf.localPortStart = dlPort;
          dlpf.localPortEnd = dlPort;
          tft->Add (dlpf);
          EpcTft::PacketFiSDNr ulpf;
          ulpf.remotePortStart = ulPort;
          ulpf.remotePortEnd = ulPort;
          tft->Add (ulpf);
          EpsBearer bearer (EpsBearer::NGBR_VIDEO_TCP_DEFAULT);
          SDNHelper->ActivateDedicatedEpsBearer (ueSDNDevs.Get (m), bearer, tft);
  ...
  // Add X2 inteface
  SDNHelper->AddX2Interface (enbNodes);

  // X2-based Handover
  SDNHelper->HandoverRequest (Seconds (0.000100), ueSDNDevs.Get (0), enbSDNDevs.Get (0), enbSDNDevs.Get (1));

  // Uncomment to enable PCAP tracing
  //p2ph.EnablePcapAll("lena-x2-handover");

  SDNHelper->EnablePhyTraces ();
  SDNHelper->EnableMacTraces ();
  SDNHelper->EnableRlcTraces ();
  SDNHelper->EnablePdcpTraces ();
  Ptr<RadioBearerStatsCalculator> rlcStats = SDNHelper->GetRlcStats ();
  rlcStats->SetAttribute ("EpochDuration", TimeValue (Seconds (0.0005)));
  Ptr<RadioBearerStatsCalculator> pdcpStats = SDNHelper->GetPdcpStats ();
...
AnimationInterface anim ("S+MN intersystem handover.xml");
  Simulator::Run ();
